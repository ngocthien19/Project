...
sort_asending = True # Trạng thái ban đầu: sắp xếp tăng dần
...
def populate_table(table, filter_value="", sort_by_weight=False, page=None):
    ...
    try:
        data = pd.read_csv(r"C:\VScode\Python\Project\database\Cleaned_Animal_Dataset.csv")
    ...
    if sort_by_weight:
        page_data = page_data.sort_values(by="Weight (kg)", ascending=sort_asending)
